...
include { RSEM_CALCULATEEXPRESSION } from "${projectDir}/modules/zhanglab/rsem/calculateexpression"
include { RSEM_MERGE_COUNTS        } from "${projectDir}/modules/local/rsem_merge_counts"
include { BAM_SORT_INDEX_SAMTOOLS  } from "${projectDir}/subworkflows/zhanglab/bam_sort_index_samtools"
...
workflow QUANTIFY_RSEM {
    ...
    BAM_SORT_INDEX_SAMTOOLS( RSEM_CALCULATEEXPRESSION.out.bam_star, fasta )
    ch_versions = ch_versions.mix(BAM_SORT_INDEX_SAMTOOLS.out.versions)

    // Merge counts across samples
    RSEM_CALCULATEEXPRESSION.out.counts_gene.collect{it[1]}.view()
    RSEM_MERGE_COUNTS (
        // Collect the second element (counts files) in the channel across all samples
        RSEM_CALCULATEEXPRESSION.out.counts_gene.collect{it[1]},    // [meta, counts]

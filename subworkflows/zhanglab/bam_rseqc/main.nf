...
include { RSEQC_GENEBODYCOVERAGE   } from "../../../modules/zhanglab/rseqc/genebodycoverage"
include { RSEQC_INNERDISTANCE      } from "../../../modules/zhanglab/rseqc/innerdistance"
include { RSEQC_JUNCTIONANNOTATION } from "../../../modules/zhanglab/rseqc/junctionannotation"
include { RSEQC_READDISTRIBUTION   } from "../../../modules/zhanglab/rseqc/readdistribution"
include { RSEQC_READGC             } from "../../../modules/zhanglab/rseqc/readgc"
include { RSEQC_RNAFRAGMENTSIZE    } from "../../../modules/zhanglab/rseqc/rnafragmentsize"
...
// include { RSEQC_READDUPLICATION    } from "../../../modules/nf-core/rseqc/readduplication"
...
workflow BAM_RSEQC {
    ...
    genebody_coverage_txt      = Channel.empty()
    genebody_coverage_pdf      = Channel.empty()
    genebody_coverage_rscript  = Channel.empty()
    if ("genebody_coverage" in rseqc_modules) {
        RSEQC_GENEBODYCOVERAGE( ch_bam_bai, ch_bed )
        genebody_coverage_txt     = RSEQC_GENEBODYCOVERAGE.out.txt
        genebody_coverage_pdf     = RSEQC_GENEBODYCOVERAGE.out.pdf
        genebody_coverage_rscript = RSEQC_GENEBODYCOVERAGE.out.rscript
    ...
    readgc_xls     = Channel.empty()
    readgc_rscript = Channel.empty()
    readgc_pdf     = Channel.empty()
    if ("read_gc" in rseqc_modules) {
        RSEQC_READGC( ch_bam )
        readgc_xls     = RSEQC_READGC.out.xls
        readgc_rscript = RSEQC_READGC.out.rscript
        readgc_pdf     = RSEQC_READGC.out.pdf
    }

    // Run RSeQC RNA_fragment_size.py
    fragsize_txt = Channel.empty()
    if ("rna_fragment_size" in rseqc_modules) {
        RSEQC_RNAFRAGMENTSIZE( ch_bam_bai, ch_bed )
        fragsize_txt = RSEQC_RNAFRAGMENTSIZE.out.txt
        ch_versions = ch_versions.mix(RSEQC_RNAFRAGMENTSIZE.out.versions.first())
    }
    ...
    genebody_coverage_txt           // channel: [ val(meta), txt ]
    genebody_coverage_pdf           // channel: [ val(meta), psd ]
    genebody_coverage_rscript       // channel: [ val(meta), r   ]
    ...
    readgc_xls                      // channel: [ val(meta), xls ]
    readgc_rscript                  // channel: [ val(meta), r   ]
    readgc_pdf                      // channel: [ val(meta), pdf ]

    fragsize_txt                    // channel: [ val(meta), txt ]
